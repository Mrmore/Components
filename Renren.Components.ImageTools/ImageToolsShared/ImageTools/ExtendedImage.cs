...
namespace ImageTools
{
    ...
    public sealed partial class ExtendedImage : ImageBase
    {
        ...
        public async Task SetSource(Stream stream)
        {
                                    #if !NETFX_CORE
        ...
        private async Task Load(Stream stream)
        {
            ...
            try
            {
                ...
                if (decoders.Count > 0)
                {
                    ...
                    if (maxHeaderSize > 0)
                    {
                        ...
                        if (decoder != null)
                        {
                            await decoder.Decode(this, stream);
                            IsLoading = false;
                        }
                    }
                }
            ...
            {
                Debug.WriteLine("ExtenedImages ===> " + e.ToString());
            }
            finally
            {
                stream.Dispose();
            ...
            var unused = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        if (await manager.Contains(uri.ToString()))
                        {
                            var resourceStream = await manager.GetData(uri.ToString());

                            Stream stream = new MemoryStream(resourceStream);
                            await LoadAsync(stream);
                        }
                         else
                        {
                            using (var resourceStream = await Extensions.GetStreamFromUri(uri))
                            {
                                ...
                                using (MemoryStream ms = new MemoryStream())
                                {
                                    ...
                                    await manager.Add(uri.ToString(), ms.ToArray());
                                }
                                await LoadAsync(resourceStream);
        ...
        {
            CacheConfigSetting setting = CacheConfigSetting.CreatSetting("image");
            CacheManager instance = await CacheManager.CreateInitedInstance(setting,setting.Key);
            manager = instance;
        }
    }
}
