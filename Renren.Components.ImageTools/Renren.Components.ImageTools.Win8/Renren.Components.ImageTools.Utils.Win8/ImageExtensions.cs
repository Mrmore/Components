...
namespace ImageTools
{
    ...
    public static class ImageExtensions
    {
        ...
        public static async Task<Stream> ToStream(this ExtendedImage image)
        {
            return await ToStream(image, null);
        }
        ...
        /// </exception>
        public static async Task<Stream> ToStream(this ExtendedImage image, IImageFilter filter)
        {
            ...
            try
            {
                ...
                await encoder.Encode(temp, memoryStream);
        ...
        public static WriteableBitmap ToBitmap(this ImageBase image, IImageFilter filter)
        {
            ...
            if (pixels != null)
            {
                var raster = bitmap.PixelBuffer.GetPixels();

                Buffer.BlockCopy(pixels, 0, raster.Bytes, 0, pixels.Length);
                raster.UpdateFromBytes();

                if (raster != null && image.NeedRaster)
                {
                    for (int i = 0; i < raster.PixeleLen; i++)
                    {
                        int abgr = raster[i];
                        int a = (abgr >> 24) & 0xff;
        ...
        public static ExtendedImage ToImage(this WriteableBitmap bitmap)
        {
            ...
            try
            {
                ...
                //var byteProvier = WinRTXamlToolkit.Imaging.IBufferExtensions.GetPixels(bitmap.PixelBuffer);
                var raster = bitmap.PixelBuffer.GetPixels();
        ...
        /// </exception>
        public static async Task WriteToStream(this ExtendedImage image, Stream stream)
        {
                                    #if !NETFX_CORE
            ...
            await encoder.Encode(image, stream);
        }
#endif
        ...
        /// 	<para><paramref name="stream"/> is null (Nothing in Visual Basic).</para>
        /// </exception>
        public static async Task WriteToStream(this ExtendedImage image, Stream stream, string fileName)
        {
                                    #if !NETFX_CORE
            Contract.Requires<ArgumentNullException>(image != null, "Image cannot be null.");
            Contract.Requires<ArgumentException>(image.IsFilled, "Image has not been loaded.");
