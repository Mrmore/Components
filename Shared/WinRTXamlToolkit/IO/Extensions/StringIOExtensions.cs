...
namespace Renren.Components.IO
{
    ...
    public static class StringIOExtensions
    {
        ...
        /// <summary>
        /// Append byte array to current file
        /// </summary>
        /// <param name="file">The storage file</param>
        /// <param name="data">The byte array</param>
        /// <returns>If successed returns the actual lenght</returns>
        public static async Task<int> AppendBytesAsync(this StorageFile file, byte[] data)
        {
            using (var stream = await file.OpenAsync(FileAccessMode.ReadWrite))
            {
                ulong offset = stream.Size;
                using (BinaryWriter writer = new BinaryWriter(stream.AsStream()))
                {
